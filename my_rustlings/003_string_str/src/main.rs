...
mod tests {
    ...
    #[test]
    fn t005_test_longest_word() {
        assert_eq!(longest_word("the quick brown fox"), "quick");
        assert_eq!(longest_word(""), "");
        assert_eq!(longest_word("a bb ccc dd"), "ccc");
        assert_eq!(longest_word("tie ties"), "ties"); // first-max on ties: "tie"(3) vs "ties"(4)
        assert_eq!(longest_word("abcd wxyz"), "abcd"); // true tie, length 4 both -> first one
    }

    #[test]
    fn t006_test_safe_slice() {
        let s = "hello";
        assert_eq!(safe_slice(s, 0, 5), Some("hello"));
        assert_eq!(safe_slice(s, 1, 3), Some("el"));
        assert_eq!(safe_slice(s, 0, 10), None); // out of bounds
        assert_eq!(safe_slice(s, 3, 1), None); // start > end

        let unicode = "héllo"; // 'é' is 2 bytes, so byte indices shift after it
        assert_eq!(safe_slice(unicode, 0, 1), Some("h"));
        assert_eq!(safe_slice(unicode, 1, 2), None); // lands mid-char-boundary of 'é'
        assert_eq!(safe_slice(unicode, 1, 3), Some("é"));
    }

    #[test]
    fn t007_test_reverse_words() {
    ...
    #[test]
    fn t008_test_byte_vs_char_len() {
        assert_eq!(byte_vs_char_len("hello"), (5, 5));
        assert_eq!(byte_vs_char_len(""), (0, 0));
        assert_eq!(byte_vs_char_len("héllo"), (6, 5)); // 'é' = 2 bytes, 1 char
        assert_eq!(byte_vs_char_len("日本語"), (9, 3)); // each char = 3 bytes
    }

    #[test]
    fn t009_test_append_exclamation() {
