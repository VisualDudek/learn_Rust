...
//! # `Vec<T>` Practice: 10 Beginner Exercises — SOLUTIONS
//!
//! This is the completed reference solution for `vec_exercises.rs`. Every
//! `// TODO` has been filled in, with a short comment on *how* the
//! solution works and *why* that's the idiomatic choice. Run `cargo test`
//! (or `rustc --test vec_exercises_solutions.rs && ./vec_exercises_solutions`)
...
fn ex1_create_and_push() -> Vec<i32> {
    // Start empty — no heap allocation happens until the first push.
    let mut v = Vec::new();
    v.push(1);
    v.push(2);
    v.push(3);
    v.push(4);
    v.push(5);
    // `v` is returned by value: ownership moves out to the caller, so
    // there's no dangling reference or extra copy — the same heap buffer
    // just changes hands.
    v
}
...
fn ex2_with_capacity() -> Vec<i32> {
    // Reserves a buffer big enough for 10 i32s in one allocation, but the
    // vector itself still reports len() == 0 — capacity != length.
...
fn ex3_pop_last(numbers: &mut Vec<i32>) -> Option<i32> {
    // pop() already returns Option<i32> itself — on an empty vector it
    // returns None instead of panicking, so we just forward its result.
    numbers.pop()
}
...
fn ex4_safe_get(numbers: &[i32], index: usize) -> Option<i32> {
    // .get(index) yields Option<&i32>; .copied() turns that into
    // Option<i32> since i32 is Copy — cheap, no ownership transfer needed.
...
fn ex5_sum_all(numbers: &[i32]) -> i32 {
    let mut total = 0;
    // `numbers` is already a `&[i32]`, so iterating it directly (rather
    // than `&numbers`) yields `&i32` items — no move, no extra borrow
    // needed. `total += n` works because `+=` is implemented for
    // `i32: AddAssign<&i32>` too, via auto-deref of `n`.
    for n in numbers {
        total += n;
    }
    total
...
fn ex6_double_all(numbers: &[i32]) -> Vec<i32> {
    // .iter() yields &i32, so `n` here is &i32; `n * 2` works because
    // Mul is implemented for &i32 too. collect() then infers Vec<i32>
...
fn ex7_middle_slice(numbers: &[i32]) -> &[i32] {
    // The range 1..len-1 excludes index 0 (the first element) and index
    // len-1 (the last element). No data is copied — this borrows a view
    // into `numbers`, so its lifetime is tied to the input slice's.
...
fn ex8_build_string_vec() -> Vec<String> {
    // Each "..." literal is a &'static str; .to_string() allocates a new
    // owned String on the heap and moves it into the vec! macro's vector.
    // The vector now owns those three Strings outright.
    vec!["rust".to_string(), "is".to_string(), "fun".to_string()]
}
...
fn ex9_filter_high_scorers(players: &[Player], min_score: u32) -> Vec<Player> {
    // .iter() yields &Player, so the closure parameter `p` is &&Player
    // once passed through filter's own reference — `p.score` still works
    // via auto-deref. .cloned() turns each &Player result into an owned
    // Player (this is why Player derives Clone), and collect() gathers
    // them into a brand-new Vec<Player>. `players` itself is never
    // mutated or moved.
    players.iter().filter(|p| p.score >= min_score).cloned().collect()
}
...
fn ex10_sort_desc_and_remove_zero_scores(players: &mut Vec<Player>) {
    // sort_by_key needs an ascending key, so wrapping the score in
    // std::cmp::Reverse flips the comparison to produce a descending
    // sort — a common idiom that avoids writing a custom comparator.
    players.sort_by_key(|p| std::cmp::Reverse(p.score));

    // retain keeps only the elements for which the closure returns true,
    // shifting survivors down and truncating the vector in place — no
    // second Vec is allocated for the result.
    players.retain(|p| p.score > 0);
}
