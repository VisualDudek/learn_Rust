//! ============================================================================
//! HashMap Fundamentals — A Progressive Exercise Set
//! ============================================================================
//!
//! This file contains 9 exercises that build fluency with `std::collections::HashMap`,
//! Rust's primary hash-table type. Each exercise isolates one concept, builds on the
//! previous one, and includes a "takeaway" comment summarizing the core lesson.
//!
//! HOW TO USE THIS FILE:
//! 1. Replace each `todo!(...)` with a working implementation.
//! 2. Run `cargo test` (or `rustc --test <file>.rs && ./<file>`) to check your work.
//! 3. Read the comment block above each function BEFORE attempting it — it explains
//!    the mental model you should bring to the problem, not just the mechanics.
//!
//! Every test in `mod tests` is written against the *correct* behavior, so it will
//! fail while the function body is still `todo!()`, and pass once you've implemented
//! it correctly.
...
pub fn ex1_create_and_insert() -> HashMap<String, i32> {
    todo!("Create an empty HashMap<String, i32>, insert (\"apples\", 5) into it, and return the map")
}
...
pub fn ex2_insert_three_fruits(
    ...
    todo!("Create an empty HashMap and insert all three (fruit, count) pairs, then return it")
}
...
pub fn ex3_get_score(scores: &HashMap<String, i32>, player: &str) -> Option<i32> {
    todo!("Look up `player` in `scores` and return their score as Option<i32> (not Option<&i32>)")
}
...
pub fn ex4_is_registered(users: &HashMap<String, u32>, username: &str) -> bool {
    todo!("Return whether `username` exists as a key in `users`")
}
...
pub fn ex5_remove_item(inventory: &mut HashMap<String, u32>, item: &str) -> Option<u32> {
    todo!("Remove `item` from `inventory` and return the removed quantity, if it existed")
}
...
pub fn ex6_sum_all_values(inventory: &HashMap<String, u32>) -> u32 {
    todo!("Iterate over `inventory` and return the sum of all quantities (values)")
}
...
pub fn ex7_ensure_default_score(scores: &mut HashMap<String, i32>, player: &str) {
    todo!("Using the entry API, ensure `player` exists in `scores` with a default value of 0 if absent (leave existing scores untouched)")
}
...
pub fn ex8_add_points(scores: &mut HashMap<String, i32>, player: &str, points: i32) {
    todo!("Using entry().and_modify().or_insert(), add `points` to player's existing score, or insert `points` as their starting score if they're new")
}
...
pub fn ex9_word_frequency(text: &str) -> HashMap<String, u32> {
    todo!("Split `text` on whitespace and return a HashMap mapping each word to how many times it appears")
}

// ============================================================================
// Tests
// ============================================================================
// These tests define the CORRECT behavior for each exercise. They will fail
// while functions are still `todo!()` (which panics), and pass once you've
// implemented each function correctly.
#[cfg(test)]
mod tests {
    use super::*;
